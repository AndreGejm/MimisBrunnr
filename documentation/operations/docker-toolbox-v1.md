...
```bash
pnpm cli sync-toolbox-client --json "{\"activeProfileId\":\"core-dev+voltagent-docs\",\"clientId\":\"codex\"}"
pnpm cli sync-toolbox-client --apply --json "{\"activeProfileId\":\"core-dev+voltagent-docs\",\"clientId\":\"codex\"}"
```
...
This VoltAgent peer is intentionally narrow: it is an optional development-only
docs convenience for Codex. It does not make Mimir the owner of VoltAgent
skills, workspace roots, or subagent behavior.

The older `core-dev+voltagent-dev` profile id remains accepted as a legacy alias
