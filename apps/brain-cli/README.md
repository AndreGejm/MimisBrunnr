...
The CLI injects a default actor context when the input omits `actor`, so the wrapper stays thin and transport-agnostic. `execute-coding-task` also defaults `repoRoot` to the current working directory when it is omitted. `version`, `--version`, and `auth-status` do not require an input payload. `freshness-status` accepts an optional JSON payload with `asOf`, `expiringWithinDays`, `corpusId`, and `limitPerCategory`. `issue-auth-token` uses JSON input to mint a short-lived issued token when `MAB_AUTH_ISSUER_SECRET` is configured.
