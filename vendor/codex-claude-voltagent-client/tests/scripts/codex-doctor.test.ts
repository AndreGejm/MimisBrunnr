...
describe("repo-level Codex doctor", () => {
  it("uses workspace defaults and warns when the native skill install is missing", () => {
    ...
    expect(result.status.configSource).toBe("workspace-override");
    expect(result.status.workspaceTrustMode).toBe("explicit-roots");
  ...
  it("falls back to the home-global config and trusts arbitrary workspaces in all-workspaces mode", () => {
    const workspaceRoot = createTempDir("codex-voltagent-doctor-global-workspace-");
    const homeRoot = createTempDir("codex-voltagent-doctor-global-home-");
    ...
    expect(result.workspaceRoot).toBe(workspaceRoot);
    expect(result.status.configPath).toBe(configPath);
    expect(result.status.configSource).toBe("home-global-default");
    expect(result.status.workspaceTrustMode).toBe("all-workspaces");
    expect(result.status.workspaceTrusted).toBe(true);
    expect(result.checks).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
