...
| --- | --- | --- | --- |
| BK-001 | Implement agent-scoped authentication and authorization | Actor-registry auth is now enforced across CLI, HTTP, MCP, and orchestrator command execution, with file-backed registry loading, rotated credentials, validity windows, issuer-secret-backed issued tokens, protected operator auth-status surfaces, HTTP token issuance and introspection routes, and CLI token introspection, but shared-rollout hardening still lacks a fuller central issuance lifecycle, revocation, and multi-operator control plane | partial |
